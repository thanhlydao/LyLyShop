...
import thanhlydao.admin.service.CustomerService;
import thanhlydao.admin.validator.CustomerValidator;
		...
		try {
			...
			if (result.hasErrors()) {
				...
				SendMail.sendMail(customer.getEmail(), "Xin Chào, Thành Viên: " + customer.getFullName(),
						"Bạn bây giờ đã có thể đăng nhập vào cửa hàng Lý Ly Shop. Hãy tha hồ mua sắm nhé :)))"
