package thanhlydao.admin.controller;
import java.io.File;
...
import thanhlydao.other.SendMail;
import thanhlydao.admin.service.ContractService;
import thanhlydao.admin.service.EmployeeService;
import thanhlydao.admin.validator.ContractValidator;
import thanhlydao.admin.validator.EmployeeValidator;
import thanhlydao.model.*;

@Controller
@Transactional
@RequestMapping(value = "admin/employee")
public class EmployeeController extends LyLyShopController{
	@Autowired
		...
		try {
			...
			if (result.hasErrors()) {
				...
				SendMail.sendMail(employee.getEmail(), "Xin Chào, Thành Viên: " + employee.getNameEmployee(),
						"Bạn đã là thành viên chính thức là nhân viên của cửa hàng Lý Ly Shop" + "\nĐịa Chỉ Email: "
	...
	public String formReset(ModelMap model, @PathVariable(value = "id") String id,
		...
		employee.setPassword("lylyshop" + new Date().getDate() + new Date().getMonth() + new Date().getYear());
		employeeService.editEmployee(employee);
		SendMail.sendMail(employee.getEmail(), "Xin Chào, Thành Viên: " + employee.getNameEmployee(),
