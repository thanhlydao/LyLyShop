...
import thanhlydao.model.Category;
import thanhlydao.model.Producer;
import thanhlydao.model.Product;
import thanhlydao.other.ShoppingCart;
...
public class UserHomeContoller {
	...
	public String feedback(ModelMap model, @RequestParam("name") String name, @RequestParam("email") String email,
		...
		try {
			...
			helper.setTo("thanhlydao97@gmail.com");
			...
			model.addAttribute("message", "Gửi phản hồi thành công. Cảm ơn bạn đã đóng góp ý kiến cho Lý Ly Shop.");
		} catch (Exception ex) {
