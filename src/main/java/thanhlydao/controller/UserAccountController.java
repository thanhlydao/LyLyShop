...
import thanhlydao.model.Customer;
import thanhlydao.other.XCaptcha;
import thanhlydao.admin.controller.LyLyShopController;

@Controller
@Transactional
@RequestMapping(value = "user")
public class UserAccountController extends LyLyShopController {
		...
		try {
			...
			if (email.equals(user.getEmail())) {
				try {
					...
					helper.setFrom("Lý Ly Shop <thanhlydao97@gmail.com>");
					helper.setTo(email);
					helper.setReplyTo("Lý Ly Shop <thanhlydao97@gmail.com>");
	...
	public String register(ModelMap model, @ModelAttribute("register") Customer customer,
		...
		if (!XCaptcha.isValid(request)) {
			model.addAttribute("message", "Mã captcha không đúng");
			return "user/register";
		}
		Session session = sessionFactory.getCurrentSession();
		try {
			...
			try {
				...
				String text = "Xin chào: " + customer.getFullName()
						+ "<br> Bạn đã đăng ký thành công tài khoản tại Lý Ly Shop"
